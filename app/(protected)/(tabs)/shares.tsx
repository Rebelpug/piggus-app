...
export default function SharesScreen() {
  ...
  const renderGroupItem = ({
    ...
    item: ExpenseGroupWithDecryptedData;
  }) => {
    if (!item || !item.data) {
      return null;
    }
              ...
              <View
                style={[
                  styles.statusIcon,
                  {
                    backgroundColor:
                      getGroupStatusColor(
                        item.membership_status || "confirmed",
                      ) + "20",
                  },
                ]}
              >
                <View
                  style={[
                    styles.statusDot,
                    {
                      backgroundColor: getGroupStatusColor(
                        item.membership_status || "confirmed",
                      ),
                    },
                  ]}
  ...
  const renderPortfolioItem = ({
    ...
    item: PortfolioWithDecryptedData;
  }) => {
    if (!item || !item.data) {
      return null;
    }
    ...
    const currency =
      item.investments?.[0]?.data?.currency ||
