...
export default function EditRecurringExpenseScreen() {
    ...
    const [selectedIntervalIndex, setSelectedIntervalIndex] = useState<IndexPath>(new IndexPath(2)); // Default to monthly
    ...
    useEffect(() => {
        if (!recurringExpenseId || !groupId || !recurringExpenses || !expensesGroups) return;
        ...
        // Set currency index
        const currencyIndex = CURRENCIES.findIndex(cur => cur.value === foundRecurringExpense.data.currency);
        setSelectedCurrencyIndex(new IndexPath(currencyIndex >= 0 ? currencyIndex : 0));

        // Set interval index
        const intervalIndex = RECURRING_INTERVALS.findIndex(int => int.value === foundRecurringExpense.data.interval);
        setSelectedIntervalIndex(new IndexPath(intervalIndex >= 0 ? intervalIndex : 2));

        // Set payer index
        const payerIndex = group.members.findIndex(m => m.user_id === foundRecurringExpense.data.payer_user_id);
        setSelectedPayerIndex(payerIndex >= 0 ? new IndexPath(payerIndex) : undefined);

        // Set split method index
        const splitMethodIndex = SPLIT_METHODS.findIndex(sm => sm.value === foundRecurringExpense.data.split_method);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const updatedRecurringExpenseData: RecurringExpenseData = {
                ...
                interval: selectedInterval.value as 'daily' | 'weekly' | 'monthly' | 'yearly',
