...
import { StyleSheet, ScrollView, Alert, TouchableOpacity, View, Modal, TextInput } from 'react-native';
import {
    Layout,
    Text,
    Card,
...
export default function BudgetingCategoriesScreen() {
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            const baseCategory = BASE_EXPENSE_CATEGORIES.find(cat => cat.id === editingCategory);
            if (baseCategory) {
                ...
                const addedIndex = newOverrides.added.findIndex(cat => cat.id === editingCategory);
                if (addedIndex !== -1) {
                    ...
                    onPress: async () => {
                        ...
                        try {
                            ...
                            if (baseCategory) {
                                ...
                                newOverrides.added = newOverrides.added.filter(cat => cat.id !== categoryId);
                            }
    ...
    const handleRestoreCategory = async (categoryId: string) => {
        ...
        try {
            const newOverrides = { ...currentOverrides };
            newOverrides.deleted = newOverrides.deleted.filter(id => id !== categoryId);
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.emojiScroll}>
                {EMOJI_SUGGESTIONS.map((emoji, index) => (
                    <TouchableOpacity
                        ...
                        style={[
                            styles.emojiButton,
                            { backgroundColor: categoryIcon === emoji ? colors.primary + '20' : colors.border },
                            { borderColor: categoryIcon === emoji ? colors.primary : colors.border }
                        ]}
            ...
            <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
                ...
                <View style={[styles.section, { backgroundColor: colors.card, shadowColor: colors.text }]}>
                    <View style={styles.sectionHeader}>
                        <Text style={[styles.sectionTitle, { color: colors.text }]}>Active Categories</Text>
                        <TouchableOpacity
                ...
                {currentOverrides.deleted.length > 0 && (
                    <View style={[styles.section, { backgroundColor: colors.card, shadowColor: colors.text }]}>
                        <Text style={[styles.sectionTitle, { color: colors.text }]}>Deleted Categories</Text>
                        ...
                        {currentOverrides.deleted.map((categoryId) => {
                            ...
                            return (
                                <View key={categoryId} style={[styles.categoryRow, { opacity: 0.6 }]}>
                                    <View style={styles.categoryInfo}>
                                        <Text style={styles.categoryIcon}>{baseCategory.icon}</Text>
                                        <Text style={[styles.categoryName, { color: colors.text }]}>{baseCategory.name}</Text>
                                        <View style={[styles.deletedBadge, { backgroundColor: colors.error + '20' }]}>
                                            <Text style={[styles.deletedText, { color: colors.error }]}>Deleted</Text>
                                        </View>
                                    </View>
                                    <TouchableOpacity
                                        style={[styles.restoreButton, { backgroundColor: colors.success + '20' }]}
