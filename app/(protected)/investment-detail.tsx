...
export default function InvestmentDetailScreen() {
  ...
  const { investmentId, portfolioId } = useLocalSearchParams<{
    investmentId: string;
    portfolioId: string;
  }>();
        ...
        { text: t("investmentDetail.cancel"), style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                t("investmentDetail.error"),
                t("investmentDetail.deleteInvestmentFailed"),
              );
            }
          },
        },
        ...
        >
            ...
            <View style={styles.headerContent}>
              <View style={styles.headerLeft}>
                ...
                <Text style={[styles.portfolioName, { color: colors.icon }]}>
                  {portfolio.data?.name ||
                    t("investmentDetail.unknownPortfolio")}{" "}
                ...
                >
                  ...
                  <Text
                    style={[
                      ...
                      {
                        color:
                          returns.totalGainLoss >= 0 ? "#4CAF50" : "#F44336",
                      },
                    ]}
                  >
                    {returns.totalGainLoss >= 0 ? "+" : ""}
                    {returns.totalGainLossPercentage.toFixed(2)}%
                  </Text>
                </View>
              </View>
            </View>
          ...
          <Card style={[styles.detailCard, { backgroundColor: colors.card }]}>
            <Text style={[styles.sectionTitle, { color: colors.text }]}>
              {t("investmentDetail.performance")}
            </Text>
            <View style={styles.detailRow}>
              <Text style={[styles.detailLabel, { color: colors.icon }]}>
                {t("investmentDetail.totalInvestment")}
              </Text>
            ...
            <View style={styles.detailRow}>
              <Text style={[styles.detailLabel, { color: colors.icon }]}>
                {t("investmentDetail.currentValue")}
              </Text>
              <Text style={[styles.detailValue, { color: colors.text }]}>
                {formatCurrency(returns.totalValue, investment.data.currency)}
              </Text>
            ...
            <View style={styles.detailRow}>
              <Text style={[styles.detailLabel, { color: colors.icon }]}>
                {t("investmentDetail.totalGainLoss")}
              </Text>
              <Text
                style={[
                  styles.detailValue,
            ...
            <View style={styles.detailRow}>
              <Text style={[styles.detailLabel, { color: colors.icon }]}>
                {t("investmentDetail.totalGainLossPercentage")}
              </Text>
              <Text
                style={[
                  styles.detailValue,
            ...
            <View style={styles.detailRow}>
              <Text style={[styles.detailLabel, { color: colors.icon }]}>
                {t("investmentDetail.estimatedYearlyGainLoss")}
              </Text>
              <Text
                style={[
                  styles.detailValue,
                  {
                    color:
                      returns.estimatedYearlyGainLoss >= 0
                        ? "#4CAF50"
                        : "#F44336",
                  },
                ]}
              >
                {returns.estimatedYearlyGainLoss >= 0 ? "+" : ""}
                {formatCurrency(
                  returns.estimatedYearlyGainLoss,
                  investment.data.currency,
                )}{" "}
                ({returns.estimatedYearlyGainLoss >= 0 ? "+" : ""}
                {returns.estimatedYearlyGainLossPercentage.toFixed(2)}%)
              </Text>
            </View>
            ...
            {returns.dividendsInterestEarned > 0 && (
              <View style={styles.detailRow}>
                <Text style={[styles.detailLabel, { color: colors.icon }]}>
                  {t("investmentDetail.dividendsInterestEarned")}
                </Text>
                <Text style={[styles.detailValue, { color: "#4CAF50" }]}>
                  {formatCurrency(
                    returns.dividendsInterestEarned,
                    investment.data.currency,
                  )}{" "}
                  ({returns.dividendsInterestEarnedPercentage.toFixed(2)}%)
                </Text>
              </View>
            )}
          </Card>
          ...
          <Card style={[styles.detailCard, { backgroundColor: colors.card }]}>
            <Text style={[styles.sectionTitle, { color: colors.text }]}>
              {t("investmentDetail.details")}
            </Text>
            <View style={styles.detailRow}>
              <Text style={[styles.detailLabel, { color: colors.icon }]}>
                {t("investmentDetail.type")}
              </Text>
              <Text style={[styles.detailValue, { color: colors.text }]}>
                {getTypeLabel(investment.data.type)}
              </Text>
            ...
            <View style={styles.detailRow}>
              <Text style={[styles.detailLabel, { color: colors.icon }]}>
                {t("investmentDetail.purchasePrice")}
              </Text>
              <Text style={[styles.detailValue, { color: colors.text }]}>
                {formatCurrency(
            ...
            {investment.data.current_price && (
              <View style={styles.detailRow}>
                <Text style={[styles.detailLabel, { color: colors.icon }]}>
                  {t("investmentDetail.currentPrice")}
                </Text>
                <Text style={[styles.detailValue, { color: colors.text }]}>
                  {formatCurrency(
                    investment.data.current_price,
            ...
            {supportsInterest && investment.data.interest_rate && (
              <View style={styles.detailRow}>
                <Text style={[styles.detailLabel, { color: colors.icon }]}>
                  {t("investmentDetail.interestRate")}
                </Text>
                <Text style={[styles.detailValue, { color: colors.text }]}>
                  {investment.data.interest_rate}%{" "}
                  {t("investmentDetail.perYear")}
                </Text>
              </View>
            )}
            ...
            <View style={styles.detailRow}>
              <Text style={[styles.detailLabel, { color: colors.icon }]}>
                {t("investmentDetail.purchaseDate")}
              </Text>
              <Text style={[styles.detailValue, { color: colors.text }]}>
            ...
            {isBond && investment.data.maturity_date && (
              <>
                <View style={styles.detailRow}>
                  <Text style={[styles.detailLabel, { color: colors.icon }]}>
                    {t("investmentDetail.maturityDate")}
                  </Text>
                  <Text style={[styles.detailValue, { color: colors.text }]}>
                ...
                <View style={styles.detailRow}>
                  ...
                  <Text
                    style={[
                      ...
                      {
                        color:
                          getBondStatus() === "matured" ? "#FF9800" : "#4CAF50",
                      },
                    ]}
                  >
                    {getBondStatus() === "matured"
                      ? t("investmentDetail.matured")
                      : t("investmentDetail.active")}
                  </Text>
                </View>
                {getDaysToMaturity() !== null &&
                  getBondStatus() === "active" && (
                    <View style={styles.detailRow}>
                      <Text
                        style={[styles.detailLabel, { color: colors.icon }]}
                      >
                        {t("investmentDetail.daysToMaturity")}
                      </Text>
                      <Text
            ...
            {investment.data.last_updated && (
              <View style={styles.detailRow}>
                <Text style={[styles.detailLabel, { color: colors.icon }]}>
                  {t("investmentDetail.lastUpdated")}
                </Text>
                <Text style={[styles.detailValue, { color: colors.text }]}>
            ...
            {investment.data.exchange_market && (
              <View style={styles.detailRow}>
                <Text style={[styles.detailLabel, { color: colors.icon }]}>
                  {t("investmentDetail.exchangeMarket")}
                </Text>
                <Text style={[styles.detailValue, { color: colors.text }]}>
                  {investment.data.exchange_market}
                </Text>
            ...
            {investment.data.taxation !== undefined &&
              investment.data.taxation !== null && (
                <View style={styles.detailRow}>
                  <Text style={[styles.detailLabel, { color: colors.icon }]}>
                    {t("investmentDetail.taxation")}
                  </Text>
                  <Text style={[styles.detailValue, { color: colors.text }]}>
