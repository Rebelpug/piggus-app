...
interface EncryptionContextType {
  ...
  // Core functions
  initializeFromSecureStorage: () => Promise<{
...
// Provider component
export const EncryptionProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  ...
  const initializeFromSecureStorage = useCallback(async (): Promise<{
    encryptionKey: Uint8Array<ArrayBufferLike>;
    privateKey: string;
  } | null> => {
    try {
  ...
  const importExistingKeys = useCallback(
    async (
  ...
  const createEncryptionKey = useCallback(async (): Promise<
  ...
  const decryptWithEncryptionKey = useCallback(
  ...
  // Encrypt data with current encryption key
  const encryptWithExternalEncryptionKey = useCallback(
    async (encryptionKey: string, data: any): Promise<string> => {
      if (!encryptionKey) {
        console.error("Cannot encrypt: encryption not initialized");
        return "";
      }
  ...
  const decryptWithExternalEncryptionKey = useCallback(
    async (encryptionKey: string, encryptedData: string): Promise<any> => {
      if (!encryptionKey) {
        console.error("Cannot decrypt: encryption not initialized");
        throw new Error("Encryption not initialized");
      }
  ...
  const encryptWithExternalPublicKey = useCallback(
    async (recipientPublicKey: string, data: any): Promise<string> => {
      try {
  ...
  const decryptFromSenderFn = useCallback(
    async (encryptedKey: string, encryptedData: string): Promise<any> => {
      if (!privateKey) {
  ...
  const exportEncryptedPrivateKey = useCallback(
    async (
      privateKey: string,
      encryptionKey: Uint8Array,
      salt: string,
