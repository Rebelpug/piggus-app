...
export const apiFetchExpensesPaginated = async (
  ...
  try {
    ...
    const paginatedResult = await piggusApi.getExpensesForGroupPaginated(
      groupId,
      params,
    );
...
export const apiFetchExpenseGroupsOnly = async (
  ...
  success: boolean;
  data?: ExpenseGroupWithDecryptedData[];
  error?: string;
}> => {
  try {
      ...
      memberships.map(async (membership) => {
        try {
          ...
          if (!/^[A-Za-z0-9+/]*={0,2}$/.test(groupKeyString)) {
            console.error(
              "Decrypted group key does not appear to be valid base64:",
              groupKeyString.substring(0, 50),
            );
            throw new Error("Invalid group key format");
          }
          // Now decrypt the group data using the decrypted group key
          const decryptedExpenseGroupData =
            await decryptWithExternalEncryptionKey(
              groupKeyString,
              group.encrypted_data,
            );
...
export const apiCreateExpensesGroup = async (
  ...
  try {
    ...
    const encryptedGroupData = await encryptWithExternalEncryptionKey(
      encryptionKeyBase64,
      groupData,
    );

    // Encrypt the group key for the user
    const encryptedGroupKey = await encryptWithExternalPublicKey(
      publicKey,
      encryptionKeyBase64,
    );
...
export const apiInviteUserToGroup = async (
  ...
  try {
    ...
    const userMembership = groupMemberships.find(
      (membership) =>
        membership.expenses_groups.id === groupId &&
    ...
    const groupKey = await decryptWithPrivateKey(
      userMembership.encrypted_group_key,
    );
    const groupKeyString =
      typeof groupKey === "string" ? groupKey : JSON.stringify(groupKey);
...
export const apiHandleGroupInvitation = async (
  ...
  try {
    ...
    const result = await piggusApi.handleExpenseGroupInvite(groupId, {
      accept,
    });
    return {
      success: result.success,
...
export const apiAddRefund = async (
  ...
  try {
    ...
    const decryptedGroupData = await decryptWithExternalEncryptionKey(
      encryptedKey,
      groupData.encrypted_data,
    );
...
export const apiUpdateRefund = async (
  ...
  try {
    ...
    const refundIndex =
      decryptedGroupData.refunds?.findIndex(
        (r: GroupRefund) => r.id === refundId,
      ) ?? -1;
    if (refundIndex === -1) {
      return {
        success: false,
...
export const apiDeleteRefund = async (
  ...
  try {
    ...
    // Remove the refund
    const updatedRefunds = (decryptedGroupData.refunds || []).filter(
      (r: GroupRefund) => r.id !== refundId,
    );
...
export const apiBulkInsertAndUpdateExpenses = async (
  ...
  try {
    ...
    const results = await piggusApi.bulkAddUpdateExpenses(
      encryptedExpenses.map(
        ({ expenseId, encryptedData, group_id, isNew, created_at }) => ({
          id: expenseId,
    ...
    const processedExpenses = results.map(
      (result, index) =>
        ({
          ...result,
