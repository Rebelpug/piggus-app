    ...
    const decryptedPortfolios = await Promise.all(
        memberships.map(async membership => {
          try {
            ...
            const membersWithProfiles = fullPortfolioData.members.map(member => ({
              ...member,
...
export const apiCreatePortfolio = async (
  ...
  try {
    ...
    const encryptedPortfolioData = await encryptWithExternalEncryptionKey(encryptionKeyBase64, portfolioData);

    // Encrypt the portfolio key for the user
    const encryptedPortfolioKey = await encryptWithExternalPublicKey(publicKey, encryptionKeyBase64);
...
export const apiAddInvestment = async (
  ...
  try {
    ...
    const encryptedData = await encryptWithExternalEncryptionKey(portfolioKey, investmentData);
...
export const apiUpdateInvestment = async (
  ...
  try {
    ...
    const encryptedData = await encryptWithExternalEncryptionKey(portfolioKey, updatedInvestment.data);

    const investment = await piggusApi.updateInvestment(portfolioId, updatedInvestment.id, {
      encryptedData,
    });
...
export const apiHandlePortfolioInvitation = async (
  ...
  try {
    ...
    const result = await piggusApi.handlePortfolioInvite(portfolioId, { accept });
    return {
      success: result.success,
    };
  } catch (error: any) {
...
export const apiUpdatePortfolio = async (
  ...
  try {
    ...
    const encryptedData = await encryptWithExternalEncryptionKey(encryptedKey, portfolioData);
