...
export default function BudgetCard({
  ...
  const [budgetAmount, setBudgetAmount] = useState(
    userProfile?.profile?.budgeting?.budget?.amount?.toString() || "",
  );
    ...
    if (selectedMonth === "current" && recurringExpenses) {
      recurringExpenses.forEach((recurringExpense) => {
        ...
        if (
          ...
          const userParticipant = recurringExpense.data.participants.find(
            (p) => p.user_id === user?.id,
          );
          if (userParticipant && userParticipant.share_amount > 0) {
            ...
            categories[category] =
              (categories[category] || 0) + userParticipant.share_amount;
          }
        }
      });
    }
  ...
  const budgetPercentUsed =
    budgetAmount_profile > 0
      ? (currentMonthData.totalSpent / budgetAmount_profile) * 100
      : 0;
              ...
              <View style={styles.budgetRemaining}>
                <Text
                  style={[
                    ...
                    {
                      color:
                        budgetRemaining >= 0 ? colors.success : colors.error,
                    },
                  ]}
                >
