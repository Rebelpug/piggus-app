...
const PasswordPrompt: React.FC<PasswordPromptProps> = ({
  ...
  const { initializeEncryptionWithPassword, signOut, tryBiometricLogin } =
    useAuth();
  ...
  const cleanupAndRedirect = async (reason: string) => {
    try {
      ...
      try {
        ...
        console.error(
          "Failed to sign out during error recovery:",
          signOutError,
        );
      }
    }
      ...
      if (biometricResult.success) {
        ...
        if (success) {
          ...
          await cleanupAndRedirect(
            "Biometric authentication succeeded but encryption initialization failed",
          );
        }
      } else {
        await cleanupAndRedirect(
