...
export default function InvestmentItem({
  ...
  const hasInterestRate =
    item.data.interest_rate && item.data.interest_rate > 0;
  ...
  const handlePress = () => {
    if (portfolioId) {
      router.push(
        `/(protected)/investment-detail?investmentId=${item.id}&portfolioId=${portfolioId}`,
      );
    }
  };
      ...
      <View style={styles.investmentCardContent}>
        <View style={styles.investmentHeader}>
          <View style={styles.investmentMainInfo}>
            ...
            <View style={styles.investmentDetails}>
              <Text style={[styles.investmentTitle, { color: colors.text }]}>
                {item.data.name ||
                  item.data.symbol ||
                  t("investments.unknownInvestment")}
              </Text>
              <Text style={[styles.investmentSubtitle, { color: colors.icon }]}>
                {item.portfolioName || t("investments.unknownPortfolio")} •{" "}
                {item.data.quantity} {t("investments.shares")}
              </Text>
          ...
          <View style={styles.investmentAmount}>
            ...
            >
              {formatCurrency(gainLoss, investmentCurrency)} (
