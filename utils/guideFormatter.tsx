...
export class GuideFormatter {
          ...
          <View key={`list-${currentIndex}`} style={styles.listContainer}>
            {listItems.map((item, idx) => (
              <View key={idx} style={styles.listItem}>
                <Text style={[styles.listMarker, { color: this.colors.primary }]}>
  ...
  private renderInlineText(text: string): React.ReactNode {
    ...
    while (remaining.length > 0) {
      ...
      let nextMatch: { index: number; length: number; content: string; type: string } | null = null;

      if (boldMatch && boldMatch.index !== undefined) {
        const candidate = { index: boldMatch.index, length: boldMatch[0].length, content: boldMatch[1], type: 'bold' };
      ...
      if (italicMatch && italicMatch.index !== undefined) {
        const candidate = { index: italicMatch.index, length: italicMatch[0].length, content: italicMatch[1], type: 'italic' };
        if (!nextMatch || candidate.index < nextMatch.index) {
          nextMatch = candidate;
        }
      }
      ...
      if (codeMatch && codeMatch.index !== undefined) {
        const candidate = { index: codeMatch.index, length: codeMatch[0].length, content: codeMatch[1], type: 'code' };
        if (!nextMatch || candidate.index < nextMatch.index) {
          nextMatch = candidate;
        }
      }
  ...
  private renderBlockquote(text: string, key: number): JSX.Element {
    return (
      <View key={key} style={[styles.blockquote, { borderLeftColor: this.colors.primary }]}>
